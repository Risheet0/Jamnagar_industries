...
export const DailyAttendanceDetailPage: React.FC<DailyAttendanceDetailPageProps> = ({ date }) => {
  ...
  const handleExportAttendanceSheet = () => {
    const rows = processedWorkers.map(w => {
      const rec = getAttendanceForDate(w.workerId || w.id, selectedDate);
      return {
        name: w.name,
    ...
    const rowsHtml = rows.map((r, i) => `
      <tr style="background: ${i % 2 === 0 ? '#fff' : '#f8fafc'}">
        <td style="padding:7px 10px; border-bottom:1px solid #e2e8f0">${r.name}</td>
        <td style="padding:7px 10px; border-bottom:1px solid #e2e8f0; font-family:monospace; font-size:11px">${r.workerId}</td>
        <td style="padding:7px 10px; border-bottom:1px solid #e2e8f0">${r.department}</td>
        <td style="padding:7px 10px; border-bottom:1px solid #e2e8f0; font-weight:600; color:${
          r.status === 'Present' ? '#059669' : r.status === 'Absent' ? '#dc2626' : r.status === 'Half Day' ? '#d97706' : '#64748b'
        }">${r.status}</td>
        <td style="padding:7px 10px; border-bottom:1px solid #e2e8f0">${r.checkIn}</td>
        <td style="padding:7px 10px; border-bottom:1px solid #e2e8f0">${r.checkOut}</td>
        <td style="padding:7px 10px; border-bottom:1px solid #e2e8f0; font-size:11px; color:#64748b">${r.notes}</td>
      </tr>
    `).join('');
    ...
    if (!win) return;
    win.document.write(`
      <html><head><title>Attendance Sheet — ${selectedDate}</title>
      <style>
        ...
        .date { font-size: 14px; font-weight: 700; margin-top: 6px; }
        table { width: 100%; border-collapse: collapse; }
        th { background: #1e3a8a; color: #fff; padding: 8px 10px; text-align: left; font-size: 11px; text-transform: uppercase; letter-spacing: .04em; }
        .summary { display: flex; gap: 20px; margin-bottom: 16px; flex-wrap: wrap; }
        .stat { padding: 8px 14px; border-radius: 8px; font-size: 12px; font-weight: 600; }
      ...
      <div class="summary">
        <div class="stat" style="background:#ecfdf5; color:#059669">Present: ${rows.filter(r => r.status === 'Present').length}</div>
        <div class="stat" style="background:#fef2f2; color:#dc2626">Absent: ${rows.filter(r => r.status === 'Absent').length}</div>
        <div class="stat" style="background:#fffbeb; color:#d97706">Half Day: ${rows.filter(r => r.status === 'Half Day').length}</div>
        <div class="stat" style="background:#f0f9ff; color:#0284c7">On Leave: ${rows.filter(r => r.status === 'On Leave').length}</div>
        <div class="stat" style="background:#f8fafc; color:#64748b">Not Marked: ${rows.filter(r => r.status === 'Not Marked').length}</div>
      </div>
      <table>
        <thead><tr>
          <th>Worker Name</th><th>ID</th><th>Department</th><th>Status</th><th>Check-In</th><th>Check-Out</th><th>Notes</th>
        </tr></thead>
        <tbody>${rowsHtml}</tbody>
      </table>
      <div style="margin-top:32px; display:flex; justify-content:space-between; font-size:11px; color:#64748b">
        <div>Prepared by: ___________________________</div>
        <div>Supervisor Signature: ___________________________</div>
      </div>
      <div style="margin-top:20px; text-align:center; font-size:10px; color:#94a3b8">
        Generated by ${companyProfile.name} ERP • ${new Date().toLocaleDateString('en-IN')}
      </div>
      </body></html>
    `);
    win.document.close();
        ...
        actions={
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <Button
              ...
              title="Print / Export attendance sheet as PDF"
            >
              Export Sheet
            </Button>
            <Button
